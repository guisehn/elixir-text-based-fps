defmodule TextBasedFPS.GameTestUtils do
  ...
  def create_player(player_key, name \\ nil) do
    unless Process.Players.get_player(player_key) do
      Process.Players.add_player(player_key)
      Process.Players.update_player(player_key, &%{&1 | name: name || player_key})
    end
  ...
  def create_room(room_name, room_map \\ nil) do
    ...
    if room_map do
      Process.Room.update(room_name, &%{&1 | game_map: GameMap.Builder.build(room_map)})
    end
  ...
  def join_room(player_key, room_name) do
    unless Process.Players.get_player(player_key),
